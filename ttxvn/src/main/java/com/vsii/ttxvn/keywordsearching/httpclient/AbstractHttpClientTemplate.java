/*
 * FILENAME
 *     AbstractHttpClientTemplate.java
 *
 * FILE LOCATION
 *     $Source$
 *
...
import com.vsii.ttxvn.keywordsearching.httpclient.auth.Credentials;
import com.vsii.ttxvn.keywordsearching.httpclient.auth.NTCredentials;
 ...
 * @author phunv
 **/
public abstract class AbstractHttpClientTemplate<T> implements
	...
	public void afterPropertiesSet() throws Exception {
		...
		for (Credentials credentials : lCredentials) {
			AuthScope authScope = new AuthScope(credentials.getAuthScopeHost(),
					credentials.getAuthScopePort(), AuthScope.ANY_REALM);
			...
			if (credentials instanceof NTCredentials) {
				...
				httpCredentials = new UsernamePasswordCredentials(
						credentials.getUserName(), credentials.getPassword());
			}
	 ...
	 *            Callback with HTTP method's response.
	 */
	public abstract void executePostMethod(String uri, T requestPayload,
	...
	protected void processHttpMethodParams(HttpMethod httpMethod,
		...
		if (hParams != null) {
			...
			if (httpMethod instanceof GetMethod) {
				((GetMethod) httpMethod).setQueryString(lParams
						.toArray(new NameValuePair[] {}));
			} else if (httpMethod instanceof PostMethod) {
				((PostMethod) httpMethod).setRequestBody(lParams
						.toArray(new NameValuePair[] {}));
			}
		}
	}
	 ...
	 * 
	 * @param httpMethod
	 *            <code>HttpMethod</code> to validate.
	 */
